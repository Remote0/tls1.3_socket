...
static int mychar_probe( struct platform_device *pdev){
    ...
    */
    ...
    printk(KERN_INFO "My-char: asssigned correctly with major number %d and minor number %d\n", MAJOR(first), MINOR(first));
    ...
    printk(KERN_INFO "My-char: initialize cdev correctly\n");
    // ************************ NORMAL Device driver *************************** //
    ...
    if (!r_irq) {
        ...
        printk(KERN_INFO "My-char: base address - 0x%08x\n", (unsigned int) aes_gcm_base_addr);
        printk(KERN_INFO "My-char: reg ictrl - 0x%08x\n", (unsigned int) aes_gcm_ictrl);
        printk(KERN_INFO "My-char: reg iresetn - 0x%08x\n", (unsigned int) aes_gcm_iresetn);
        return 0;
    }
    lp->irq = r_irq->start;
    ...
    */
    printk("My-char: IRQ number found (ignore this) \n");
    printk("My-char: mychar 0x%08x mapped to 0x%08x, irq=%d\n",
        (unsigned int __force) lp->mem_start,
        (unsigned int __force) lp->base_addr,
        lp->irq);
    /* Configuring pointers to the IP registers */
