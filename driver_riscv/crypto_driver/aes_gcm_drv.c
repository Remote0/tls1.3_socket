...
static uint32_t* text; //should be large enough to store input data, or can be just a pointer
...
int hw_aes_gcm_encrypt(struct aes_gcm_data* aes_gcm, const uint32_t mode, uint32_t* output, uint32_t* input, uint32_t input_length, uint32_t* key, const uint32_t key_len, uint32_t* iv, uint32_t* aad, const size_t aad_len, uint32_t* tag){
    ...
    printk(KERN_INFO "aes-gcm: Run aes-gcm module in ENCRYPTION mode\n");
    //Generate HashKey
    ...
    printk(KERN_INFO "aes-gcm: Set IV - 0x%08x%08x%08x\n", *(iv), *(iv+1), *(iv+2));
    ...
    //hw_aes_gcm_wait_ready(aes_gcm); --read tag already wait for tag to be done
...
static ssize_t dev_read(struct file* filep, char* buffer, size_t len, loff_t* offset){
    ...
    if(error_count == 0){
        ...
        return(send_byte=0); // clear the position to the start and return 0;
    }else{
...
static ssize_t dev_write(struct file* filep, const char* buffer, size_t len, loff_t* offset){
    ...
    if(ker_buff_len < 2) { //select buffer, len <= 2 bytes
        ...
        printk("aes-gcm: Select buffer: %d\n", sel_buffer);

        if(sel_buffer == 5) {
            //call encryption mode
            output = (uint32_t*)kmalloc(sizeof(uint32_t) * text_len, GFP_KERNEL);
            tag = (uint32_t*)kmalloc(sizeof(uint32_t) * 4, GFP_KERNEL);
            hw_aes_gcm_encrypt(lp, 1, output, text, text_len, key, key_len, iv, aad, aad_len, tag);
        }
        if(sel_buffer == 6) {
            //call decryption mode
            output = (uint32_t*)kmalloc(sizeof(uint32_t) * text_len, GFP_KERNEL);
            hw_aes_gcm_encrypt(lp, 0, output, text, text_len, key, key_len, iv, aad, aad_len, tag);
        }
    }
    else {
        switch(sel_buffer) {
            case 0:
                ...
                printk(KERN_INFO "aes-gcm: Set key - 0x%08x%08x%08x%08x\n", *(key), *(key+1), *(key+2), *(key+3));
                printk(KERN_INFO "aes-gcm: Set key - 0x%08x%08x%08x%08x\n", *(key+4), *(key+5), *(key+6), *(key+7));
                printk(KERN_INFO "aes-gcm: key is in kernel space with length - %d\n", key_len);
                break;
            case 1:
                //Get data from user to kernel spaces
                ...
                printk(KERN_INFO "aes-gcm: iv is in kernel space");
                break;
            case 2:
                //Get data from user to kernel spaces
                text = (uint32_t*)kmalloc(sizeof(uint32_t) * ker_buff_len, GFP_KERNEL);
                for(i=0; i<ker_buff_len; i=i+1)
                    text[i] = ker_buff[i];
                //kfree(ker_buff);
                
                text_len = ker_buff_len; //len in bytes
                for (i = 0; i < text_len; i++)
                {
                    printk("aes-gcm: input - 0x%08x%08x%08x%08x\n", text[i], text[i+1], text[i+2], text[i+3]);
                    i = i + 3;
                }
                printk(KERN_INFO "aes-gcm: input is in kernel space with length %d", text_len);
                break;
            case 3:
                //Get data from user to kernel spaces
                ...
                printk(KERN_INFO "aes-gcm: Set aad - 0x%08x%08x%08x%08x\n", *(aad), *(aad+1), *(aad+2), *(aad+3));
                printk(KERN_INFO "aes-gcm: Set aad - 0x%08x%08x%08x%08x\n", *(aad+4), *(aad+5), *(aad+6), 0x0);
                printk(KERN_INFO "aes-gcm: aad is in kernel space with length %d", aad_len);
                break;
            case 4:
                //Get data from user to kernel spaces
                ...
                printk(KERN_INFO "aes-gcm: tag is in kernel space");
                break;
            default:
                printk(KERN_INFO "aes-gcm: Cannot decide what to do - Select again");
        }
    }
